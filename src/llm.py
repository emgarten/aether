...
def query_llm(prompt: str, system=SYSTEM_PROMPT, max_tokens=1024) -> str:
    ...
    if hasattr(completion, "usage") and completion.usage is not None:
        logging.debug(f"Total tokens: {completion.usage.total_tokens} Prompt tokens: {completion.usage.prompt_tokens} Completion tokens: {completion.usage.completion_tokens}")
